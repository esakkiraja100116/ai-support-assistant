...
Full setup/run/test details live in each service's own README.
...
**One real bug this surfaced, worth calling out**: articles were originally embedded on `question + answer` combined, which diluted the vector enough that the exact seeded question "How do I sell my gold?" only scored 0.74 against itself — just under the 0.75 threshold, so an exact match was (wrongly) treated as ungrounded. Switching to embedding the **question text alone** fixed it (exact match → 1.0 similarity) — because customer messages arrive as questions, question-to-question similarity is a cleaner signal than question-to-(question+answer) similarity. This is why `scripts/seed.py` embeds `question`, not `f"{question} {answer}"`.
...
- **Retrieval quality**: chunk longer articles instead of one-embedding-per-FAQ, add reranking for borderline-similarity results, and surface citations in the UI (the backend already returns `sources`, the frontend doesn't show them yet).
